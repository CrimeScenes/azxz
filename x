...
local function ease(t, easingStyle)
    local easingFunctions = {
        Linear = function(t) return t end,
        Sine = function(t) return math.sin(t * math.pi / 2) end,
        Back = function(t) return t * t * ((2.70158 + 1) * t - 2.70158) end,
    ...
    return easingFunctions[easingStyle](t)
end

-- Function to smoothly transition the camera position with easing
local function smoothTransition(currentPosition, targetPosition, smoothingFactor, easingStyle)
    local startTime = tick() -- Capture the start time of the transition
    local duration = smoothingFactor or 0.5 -- Default duration if not specified
    
    -- Move the camera smoothly with easing style
    return function()
        local elapsedTime = tick() - startTime
        local t = math.min(elapsedTime / duration, 1)
        
        -- Apply easing
        local easedT = ease(t, easingStyle)
        
        -- Interpolate between current and target position
        return currentPosition:Lerp(targetPosition, easedT)
    end
end
    ...
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if predictedPosition then
                ...
                -- Check if smoothness is enabled
                if getgenv().CamLock.Normal.SmoothnessEnabled then
                    -- Apply easing transition based on the selected easing style
                    local transition = smoothTransition(Camera.CFrame.Position, predictedPosition, getgenv().CamLock.Normal.Smoothness, getgenv().CamLock.EasingStyle)
                    Camera.CFrame = CFrame.new(transition())  -- Apply the eased transition
                else
...
RunService.RenderStepped:Connect(function()
    ...
    elseif LastTarget and LastTarget.Character then
        ...
        if IsVisible(lastTargetPos) then
            -- Apply smooth transition to the last target's position using easing
